...
if os.path.exists(path):
    ...
    st.info("results/threat_scores.csv illa. threat_score.py mudhal run pannunga.")

st.subheader("Limitations")
st.markdown(
    "- Test set is small, so levels with few clips (for example Critical) give noisy percentages.\n"
    "- The CNN uses frozen ImageNet features. Fine-tuning and motion features (optical flow) may improve results.\n"
    "- The model is trained on ShanghaiTech Campus scenes. Other cameras may reduce accuracy.\n"
    "- Zone risk is set by the operator per camera, not learned from data.\n"
    "- There is no ground-truth severity label, so grading is validated through the share of truly abnormal clips per level."
)